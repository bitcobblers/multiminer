...
export const AVAILABLE_MINERS: MinerInfo[] = [
  ...
  },
  {
    ...
    owner: 'nanopool',
    repo: 'nanominer',
    assetPattern: /^nanominer-windows-\d+\.\d+\.\d+\.zip$/,
    optionsUrl: 'https://help.nanominer.org/article/169-main-settings',
    exe: 'nanominer.exe',
    getArgs: (alg, cs, url) => `-algo ${alg} -wallet ${cs} -pool1 ${url} -nocolor true -webport 60090`,
  },
  {
    ...
    owner: 'nanopool',
    repo: 'nanominer',
    assetPattern: /^nanominer-windows-\d+\.\d+\.\d+-cuda11\.zip$/,
    optionsUrl: 'https://help.nanominer.org/article/169-main-settings',
    exe: 'nanominer.exe',
    getArgs: (alg, cs, url) => `-algo ${alg} -wallet ${cs} -pool1 ${url} -nocolor true -webport 60090`,
  },
